...
namespace ThirdCaliburnApp.ViewModels
{
    public class MainViewModel : Conductor<object>, IHaveDisplayName
    {
        ...
        public MainViewModel(IWindowManager windowManager, IDialogService dialogService) // 생성자
        {
        ...
        public void SaveEmployee()
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Commons.CONNSTRING))
                {
                    ...
                    cmd.Connection = conn;
                    if (Id == 0) // Insert
                        cmd.CommandText = EmployeesTbl.INSERT_EMPLOYEES;
                    else // Update
                        cmd.CommandText = EmployeesTbl.UPDATE_EMPLOYEES;
